...
const displayCityName = document.querySelector(".city-name-display");

const reseter = document.querySelector(".new-shearch");
const closer = document.querySelector(".closer-btn");
...
const fetchCityData = (city) => {
    fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${city}`)
        ...
        .then((data) => {
            ...
            urlvar = `https://api.open-meteo.com/v1/forecast?latitude=${latvar}&longitude=${lonvar}&current=temperature_2m,apparent_temperature,relative_humidity_2m,weather_code,wind_speed_10m,wind_direction_10m,pressure_msl,cloud_cover,is_day`;
            comunicate.textContent = `results for:${city}`;
...
shearchInitate.addEventListener("click", () => {
    ...
    if (cityName === "") {
        ...
        comunicate.textContent = `Weather in ${cityName}`;
...
reseter.addEventListener("click", () => {
    ...
    comunicate.textContent = "Search For City to Start";
